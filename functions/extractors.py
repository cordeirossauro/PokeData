...
def phys_inf_extractor(soup):
    ...
    phys_inf = pd.DataFrame(
        columns=["classification", "base_egg_steps", "capture_rate", "weight", "height"]
    )
...
def weaknesses_extractor(soup):
    ...
    weaknesses_df = pd.DataFrame(
        columns=[poketype + "_weakness" for poketype in poketypes]
    )
